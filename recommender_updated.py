...
def bot():
    ...
    if incoming_msg.lower().strip() in posCat:
        responded = True
        category = incoming_msg # str.capitalize()? look at df to see format
        resp.message("Would you like to donate to a 'small', 'mid' or 'big' organization?")

    if incoming_msg.lower().strip() in posSize:
        responded = True
        size = incoming_msg # str.capitalize()? look at df to see format
        resp.message(f"You have requested {size} {category} charity recommendation")
